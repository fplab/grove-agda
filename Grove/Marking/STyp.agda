...
-- types
module Grove.Marking.STyp where
  data STyp : Set where
    num     : STyp
    unknown : STyp
    _-→_    : STyp → STyp → STyp
  ...
  module base where
    -- base types
    data _base : (τ : STyp) → Set where
      TBNum  : num base

    -- decidable base
    _base? : (τ : STyp) → Dec (τ base)
    num      base? = yes TBNum
    unknown  base? = no (λ ())
    (_ -→ _) base? = no (λ ())
  ...
  module consistency where
    ...
    -- consistency
    data _~_ : (τ₁ τ₂ : STyp) → Set where
      TCUnknown     : unknown ~ unknown
      TCBase        : {τ : STyp} → (b : τ base) → τ ~ τ
      TCUnknownBase : {τ : STyp} → (b : τ base) → unknown ~ τ
      TCBaseUnknown : {τ : STyp} → (b : τ base) → τ ~ unknown
      TCArr         : {τ₁ τ₂ τ₁' τ₂' : STyp}
                    → (τ₁~τ₁' : τ₁ ~ τ₁')
                    → (τ₂~τ₂' : τ₂ ~ τ₂')
                    → τ₁ -→ τ₂ ~ τ₁' -→ τ₂'
      TCUnknownArr  : {τ₁ τ₂ : STyp}
                    → unknown ~ τ₁ -→ τ₂
      TCArrUnknown  : {τ₁ τ₂ : STyp}
                    → τ₁ -→ τ₂ ~ unknown

    -- inconsistency
    _~̸_ : (τ₁ : STyp) → (τ₂ : STyp) → Set
    τ₁ ~̸ τ₂ = ¬ (τ₁ ~ τ₂)
  ...
  module matched where
    ...
    -- matched arrow
    data _▸_-→_ : (τ τ₁ τ₂ : STyp) → Set where
      TMAUnknown : unknown ▸ unknown -→ unknown
      TMAArr     : {τ₁ τ₂ : STyp} → τ₁ -→ τ₂ ▸ τ₁ -→ τ₂
  ...
  module meet where
    ...
    data _⊓_⇒_ : (τ₁ τ₂ τ : STyp) → Set where
      TJBase        : {τ : STyp} → (b : τ base) → τ ⊓ τ ⇒ τ
      TJUnknown     : unknown ⊓ unknown ⇒ unknown
      TJUnknownBase : {τ : STyp} → (b : τ base) → unknown ⊓ τ ⇒ τ
      TJBaseUnknown : {τ : STyp} → (b : τ base) → τ ⊓ unknown ⇒ τ
      TJArr         : {τ₁ τ₂ τ₁' τ₂' τ₁″ τ₂″ : STyp}
                     → (τ₁⊓τ₁' : τ₁ ⊓ τ₁' ⇒ τ₁″)
                     → (τ₂⊓τ₂' : τ₂ ⊓ τ₂' ⇒ τ₂″)
                     → τ₁ -→ τ₂ ⊓ τ₁' -→ τ₂' ⇒ τ₁″ -→ τ₂″
      TJUnknownArr  : {τ₁ τ₂ : STyp}
                     → unknown ⊓ τ₁ -→ τ₂ ⇒ τ₁ -→ τ₂
      TJArrUnknown  : {τ₁ τ₂ : STyp}
                     → τ₁ -→ τ₂ ⊓ unknown ⇒ τ₁ -→ τ₂

    -- decidable meet
    _⊓?_ : (τ₁ : STyp) → (τ₂ : STyp) → Dec (∃[ τ ] τ₁ ⊓ τ₂ ⇒ τ)
